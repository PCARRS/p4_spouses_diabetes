...
# Table 4 pooled logistic models
...
fit_models_one <- function(dyads, sex_label, include_behavior_terms = FALSE, imp_num = NA_integer_) {
  ...
  fit_log_or <- function(formula, data) {
  ...
  age_est <- fit_log_or(dm_incident ~ partner_dm_status + age, df_model)
  m1_est <- fit_log_or(dm_incident ~ partner_dm_status + age + edu_category + famhx_dm, df_model)
  m2_est <- if (include_behavior_terms) {
    fit_log_or(dm_incident ~ partner_dm_status + age + edu_category + famhx_dm +
  ...
  tibble::tibble(
    ...
    log_or = c(age_est$est, m1_est$est, m2_est$est),
    log_or_se = c(age_est$se, m1_est$se, m2_est$se),
...
collect_sex <- function(sex_label, include_behavior_terms) {
  ...
  utils::write.csv(
    res_tbl,
    file = paste0("analysis/psdan04_logit_models_", tolower(sex_label), ".csv"),
    row.names = FALSE
  )
...
fmt_or <- function(log_or, lcl, ucl) {
  sprintf("%.2f (%.2f - %.2f)", exp(log_or), exp(lcl), exp(ucl))
}
...
build_rows <- function(sex_label, res_tbl) {
  ...
  tibble::tibble(
    ...
    `Age adjusted OR (95% CI)` = c("1.00", fmt_or(age$log_or, age$lcl, age$ucl)),
    `Model 1 adjusted OR (95% CI)` = c("1.00", fmt_or(m1$log_or, m1$lcl, m1$ucl)),
    `Model 2 adjusted OR (95% CI)` = c("1.00", fmt_or(m2$log_or, m2$lcl, m2$ucl))
  )
}
...
utils::write.csv(
  table4_df,
  file = "analysis/psdan04_table4_spousal_baseline_dm_logistic.csv",
  row.names = FALSE
)
